  ...
  <div class="flex h-screen bg-gray-100 font-sans text-gray-900">
    ...
    >
      ...
      <nav class="flex-1 px-3 py-6 space-y-2 overflow-y-auto overflow-x-hidden">
       ...
       <router-link v-if="userRole != 'THU_THU' && userRole != 'QUAN_LY_KHO' && userRole != 'KE_TOAN'" to="/reader/home" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group">
        ...
        <template v-if="userRole === 'DOC_GIA'">
          <router-link to="/reader/library" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Thư viện sách' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          ...
          <router-link to="/reader/history" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Lịch sử mượn' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          ...
          <router-link to="/reader/profile" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Hồ sơ cá nhân' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <template v-if="userRole === 'KE_TOAN'">
          <router-link to="/accountant/fines" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Quản lý Thu Phạt' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          ...
          <router-link to="/accountant/staff" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <template v-if="userRole === 'THU_THU'">
          <router-link to="/librarian/transactions" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Nghiệp vụ Mượn/Trả' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
          ...
          <router-link to="/librarian/readers" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-bg-blue-50 hover:text-blue-700 transition-colors group">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
            </svg>
          ...
          <router-link to="/librarian/books" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-bg-blue-50 hover:text-blue-700 transition-colors group">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
            </svg>
        ...
        <template v-if="userRole === 'QUAN_LY_KHO'">
          <router-link to="/inventory/books" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors group" :title="!isSidebarOpen ? 'Quản lý Danh mục Sách' : ''">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
            </svg>
        ...
        <template v-if="userRole === 'GIAM_DOC'">
          <router-link to="/director/audit-log" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-white bg-blue-600 shadow-md">
            <svg class="w-6 h-6 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
            <span v-if="isSidebarOpen" class="ml-3 whitespace-nowrap font-bold">Audit Log (Kiểm toán)</span>
          </router-link>

          <router-link to="/director/reports" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg text-gray-700 hover:bg-blue-50 transition-colors group">
            <svg class="w-6 h-6 flex-shrink-0 text-gray-400 group-hover:text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
type Role = 'DOC_GIA' | 'THU_THU' | 'QUAN_LY_KHO' | 'KE_TOAN' | 'GIAM_DOC';


const savedRole = localStorage.getItem('role') as Role;
const userRole = ref<Role>(savedRole || 'DOC_GIA');
// Biến quản lý trạng thái đóng/mở Sidebar
